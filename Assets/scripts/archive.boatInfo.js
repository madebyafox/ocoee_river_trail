...
//boatInfo.js
//ARF	4.16.14
//Primary logic controller for flip/bounce and swim/roll decisions. Handles collisions. 
//Does not cause boat movement
//------------------------------------------------------------------------------------------------
...
function calcSum(decType: String):Vector2
{
	...
	if (decType == "flip"){
		...
		if (stability <= 5 && skill <= 6) {outlier = 1;} //always flip 
		if (health <= 2) {outlier = 1;} //always flip
		if (skill == 10) {outlier = 2;} //never flip
	}
	else if(decType == "swim"){
		total = health + rollability + roll;
		if ((roll <= 1) || (rollability <= 1)) {outlier = 1;} //always swim
		if ((roll <= 5) && (rollability <= 5)) {outlier = 1;} //always swim
		if (health<= 2) {outlier = 1;} //always swim

		else if (roll == 10) {outlier = 2;} //never swim, always roll
	}
...
function flipOrBounce(coll: Collision2D)
{
	...
	if (outlier == 1){
		...
		else if (sumTest < 20) {chance = 50;}
		else if (sumTest < 20) {chance = 50;}
		else if (sumTest < 30) {chance = 20;}
		chance = chance + hits; //increase the chance of flipping if the number of hits is higher
